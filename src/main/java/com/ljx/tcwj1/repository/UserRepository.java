...
* @ImportJar:
    JPA注解包
        资源库下载地址https://repo.spring.io/release/org/springframework/data/
        spring-data-jpa-2.1.2.RELEASE.jar
        spring-data-commons-2.1.2.RELEASE.jar
* @ApiGrammer规则：
* @Remark:
...
public interface UserRepository extends JpaRepository<UserDO, Long> {
     ...
     * @ApiGrammer规则：
            ...
            @Query( "使用实体对象及实体字段的sql语句" )
     */
    @Query(value = "SELECT sex AS sex,count(sex) AS sexNum FROM UserDO WHERE 1=1 and user_id= :userId  and if(:param1!='',email LIKE CONCAT('%',:param1,'%'),1=1) GROUP BY sex ",  nativeQuery = true)
