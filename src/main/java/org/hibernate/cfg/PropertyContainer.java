...
/**
 *
 * @Description:  解决SpringDataJpa实体类中属性顺序与数据库中生成字段顺序不一致的问题
 * @FR功能需求：
 * @ImportJar:  这里的Hibernate版本为hibernate5
 * @ApiGrammer规则：
 * @Remark:
 * @CodeBug解决:
        问题：解决 Springboot Jpa 自动创建表 和字段乱序问题
        产生原因：因为使用 jpa 自动创建表的时候，采用的是TreeMap的，我们要变成LinkedHashMap 的结构。
        解决方案：在项目下java文件夹下创建包名为： org.hibernate.cfg  ，创建类： PropertyContainer [包名，类名一定要相同哦]
